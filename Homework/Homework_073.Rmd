...
```{r}
d_raw <- read.table("d_raw",header=TRUE,sep=",", stringsAsFactors=FALSE)
str(d_raw)
summary(d_raw)
summary(d_raw$approaches)
```

```{r}
d_raw2 <- rnorm(n=3000,mean=0.2)
d_raw3 <- data.frame(1:3000, data)
names(data) <- list("ID","approaches")
d_raw4 <- data[data$approaches>0,]
str(d_raw4)
summary(d_raw4$approaches)
```
...
```{r}
p1 <- ggplot(data=d_raw3, aes(x=myVar, y=..density..)) +
  geom_histogram(color="grey60", fill="cornsilk", size=0.2) 
print(p1)
```
...
```{r}
stat3 <- stat_function(aes(x = xval, y = ..y..), fun = dunif, colour="darkgreen", n = length(data$age), args = list(min=min(data$age), max=max(data$age)))
 p1 + stat + stat2 + stat3
```
...
```{r}
gammaPars <- fitdistr(data$age,"gamma")
shapeML <- gammaPars$estimate["shape"]
rateML <- gammaPars$estimate["rate"]

stat4 <- stat_function(aes(x = xval, y = ..y..), fun = dgamma, colour="brown", n = length(data$age), args = list(shape=shapeML, rate=rateML))
 p1 + stat + stat2 + stat3 + stat4
```
